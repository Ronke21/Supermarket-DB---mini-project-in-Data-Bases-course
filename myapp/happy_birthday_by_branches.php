	...
	<head>
		...
		<body>
			...
			<?php
				...
				$sql = "SELECT `worker`.`First_Name`,`worker`.`Last_Name`,`worker`.`Date_of_Birth`, `branches`.`location`
						FROM `worker`, `branches`
						WHERE MONTH(`Date_of_Birth`) = (SELECT MONTH((CAST(NOW() AS Date))))
														AND DAY(`Date_of_Birth`) >= DAY(CAST(NOW() AS Date))
														AND `worker`.`branch`=`branches`.`branch_id`
