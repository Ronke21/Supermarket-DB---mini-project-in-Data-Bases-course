...
<html lang="en">
	<head>
		...
		<title>DELIVERIES BY BRANCH</title>
	...
	<body>
		...
		<div class="card">
			...
			<form method="post" action="?submit">
				...
				<h4>
					select branch to get all deliveries
				</h4>
			...
			<?php
				...
				if(isset($_POST['submitForm']))
				{
					...
					if(empty($_POST['branch']))
					{
						...
						echo "<h5> please select brnach </h5>";
					}

					// if the id field is not empty, execute the query
					// with the parameter $name = $_POST['branchName'];
					if(isset($_POST['branch']))
					{
						...
						$sql = "SELECT *
								...
								WHERE `branches`.`location` = '".$branchName."'
								AND `branches`.`location` = `delievers`.`Area`";
						...
						if ($result -> num_rows > 0) {
							...
							while ($row = $result -> fetch_assoc()) {
								echo "
								<tr>
									<td>".$row["Id_deliever"].	"</td>
									<td>".$row["Address_destination"].	"</td>
									<td>".$row["Order_create_time"].	"</td>
									<td>".$row["Order_shipping_time"].	"</td>
									<td>".$row["Deliever_fee"].	"</td>
									<td>".$row["Area"].	"</td>
									<td>".$row["Start_shipping"].	"</td>
									<td>".$row["End_shipping"].	"</td>
									<td>".$row["Require_cooling"].	"</td>
									<td>".$row["branch_id"].	"</td>
									<td>".$row["location"].	"</td>
