...
<html lang="en">
	<head>
		...
		<title>workers with most deliveries</title>
		...
		<body>
			...
			<?php
				...
				$sql = "SELECT count(`deliever scheduling`.`Id_deliever`) AS `number of deliveries`, `worker`.`ID`, `worker`.`First_Name`, `worker`.`Last_Name`
				        FROM `deliever scheduling`, `worker` where `deliever scheduling`.`id_worker`=`worker`.`ID`
                        GROUP BY `worker`.`ID`
                ...
                if ($result -> num_rows > 0) {
					...
					while ($row = $result -> fetch_assoc()) {
						...
						<tr>
							...
							<td>".$row["number of deliveries"]." </td>
							<td>".$row["ID"].	               " </td>
